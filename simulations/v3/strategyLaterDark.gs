/**
 * Copies default strategy, but has higher threshold for when to go
 * to towards the Tower.
 */
strategies.laterDark = Object.assign({}, strategies.default);
   ...
   */
strategies.laterDark.setDestination = function(character, gameState) {
  ...
  if (character.fightValues.max >= 7)
    goDark = true;
  else if (character.fightValues.max >= 6 && character.fightValues.ranged && character['ranged'] >= 2)
    goDark = true;
  if (goDark) {
